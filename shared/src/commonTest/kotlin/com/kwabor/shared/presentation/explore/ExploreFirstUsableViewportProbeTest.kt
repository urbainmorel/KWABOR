...
class ExploreFirstUsableViewportProbeTest {
    @Test
    fun duplicateAppearAndCommitProduceExactlyOneColdMeasurement() {
        ...
        assertEquals(PerformanceSampleKind.Cold, measurement?.sample?.sampleKind)
    ...
    @Test
    fun staleGenerationCannotCompleteTheWarmForegroundSample() {
        val probe = ExploreFirstUsableViewportProbe()
        val cold = requireNotNull(probe.onVisible(true, 1_000L))
        probe.onHidden()
        val warm = requireNotNull(probe.onVisible(true, 3_000L))

        assertEquals(PerformanceSampleKind.Warm, warm.sampleKind)
        assertNull(probe.onViewportCommitted(cold.generation, PerformanceViewportState.Content, 4_000L))
        assertEquals(
            2_000L,
            probe.onViewportCommitted(warm.generation, PerformanceViewportState.Empty, 5_000L)
                ?.durationNanoseconds,
        )
    }

    @Test
    fun backgroundOrDismissalCancelsWithoutExportingAndReentryIsWarm() {
        val probe = ExploreFirstUsableViewportProbe()
        val cold = requireNotNull(probe.onVisible(true, 1_000L))

        probe.onHidden()

        assertNull(probe.onViewportCommitted(cold.generation, PerformanceViewportState.Offline, 2_000L))
        val warm = requireNotNull(probe.onVisible(true, 3_000L))
        assertEquals(PerformanceSampleKind.Warm, warm.sampleKind)
    }
    ...
    fun deniedConsentAndClockRollbackFailClosed() {
        ...
        assertEquals(PerformanceSampleKind.Cold, firstAllowed.sampleKind)
    ...
    fun measurementMapsToTheTypedNonPiiFirebaseContract() {
        ...
        assertEquals(PerformanceSampleKind.Cold, measurement.sampleKind)
