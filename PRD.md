...
1. **Intro vidéo interactive** (premier lancement, puis une fois par nouvelle révision distante validée) : vidéo d'arrière-plan immersive, **sautable**, avec les CTA **S'inscrire / Se connecter / Continuer sans compte disponibles immédiatement** ; la vidéo ne bloque jamais l'action. Fin de vidéo, échec, « Passer » ou mouvement réduit → même surface sur image statique. Détail asset & performance en §6.9.3.
...
- **Interactive et sautable** : les CTA d'accès sont disponibles dès la première frame ; « Passer » arrête seulement la vidéo et conserve la surface d'accès sur l'image statique. Une sélection de CTA consomme l'intro avec le motif `cta_selected`. La vidéo est affichée au **premier lancement**, puis **une seule fois par nouvelle révision distante validée et précachée** ; le fallback statique est direct en mouvement réduit, en cas d'échec et lors des lancements suivants. Une révision reçue en cours de session n'interrompt jamais l'utilisateur. **Analytics : taux de skip**, uniquement si un consentement existe déjà.
...
| **Vidéo d'intro trop lourde / lente** | Premier lancement dégradé | Asset **embarqué** (indépendant du réseau), **≤ ~2–3 Mo**, H.264 muet, **sautable**, puis révisions distantes validées présentées une seule fois au lancement suivant, fallback `reduced-motion` ; analytics de skip. |
