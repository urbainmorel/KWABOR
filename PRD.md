...
1. **Intro vidéo** (premier lancement, puis une fois après installation de chaque nouvelle révision embarquée) : vidéo d'arrière-plan immersive, **sautable** ; la langue est **détectée automatiquement** pendant l'intro (§6.9.1). Fin de vidéo (ou « Passer ») → écran suivant. Une mise à jour qui conserve la même révision ne rejoue pas l'intro. Détail asset & performance en §6.9.3.
...
- **Sautable** (bouton « Passer » visible immédiatement) ; affichée au **premier lancement**, puis **une seule fois après installation d'une version Store dont la révision embarquée est strictement supérieure à la dernière présentée** (les autres lancements vont directement à l'auth ou à l'accueil) ; **fallback image statique** si `prefers-reduced-motion`. Une mise à jour applicative n'interrompt jamais une session en cours et la nouvelle révision n'est proposée qu'au lancement suivant. **Analytics : taux de skip** pour vérifier que la vidéo mérite sa place, uniquement lorsque le consentement Analytics existe déjà ; aucun événement de premier lancement n'est mémorisé avant consentement.
...
- **Écran d'intro vidéo** au premier lancement puis une fois par nouvelle révision embarquée (remplacement par release Store uniquement, sautable, H.264 muet vertical ~2–3 Mo).
