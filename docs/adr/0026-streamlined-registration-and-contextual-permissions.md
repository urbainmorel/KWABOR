# 0026 — Simplifier l'inscription et différer les permissions au contexte
...
fallback statique. L'ADR-0021 continue de régir le média embarqué : tout changement d'octets exige
une nouvelle version Android/iOS publiée dans les Stores.
...
- La gestion des consentements reste une responsabilité des Réglages et les notifications ne sont
  demandées qu'au premier besoin réellement contextuel.
- L'intro interactive augmente la densité de la première surface ; le scrim et les tests
  d'accessibilité doivent préserver la lisibilité des CTA sur chaque frame.
- Les couches plateforme conservent l'action protégée et doivent prouver la parité de replay sur
  Android et iOS.
...
- Le contrat Store-only ou la révision embarquée de l'ADR-0021 évolue.
