...
Le grant `EXECUTE` de `authenticated` est redéclaré explicitement sur ces deux wrappers. Aucun appel
serveur ne les consomme : `service_role` reste donc exclu du setter et des wrappers, au lieu de lui
ouvrir directement les tables privées `favorites` et `likes` pour entretenir un faux contrat local.
