...
class AndroidExploreFirstUsableViewportReporterTest {
    ...
    fun layoutDuplicatesAndStaleCallbacksExportOneMonotonicMeasurement() {
        ...
        val coldGeneration = requireNotNull(reporter.onVisible())

        assertEquals(coldGeneration, reporter.onVisible())
        reporter.onHidden()
        clock.now = 2_000L
        val warmGeneration = requireNotNull(reporter.onVisible())
        clock.now = 5_500L
        reporter.onViewportCommitted(coldGeneration, PerformanceViewportState.Content)
        reporter.onViewportCommitted(warmGeneration, PerformanceViewportState.Offline)
        reporter.onViewportCommitted(warmGeneration, PerformanceViewportState.Offline)

        assertEquals(1, measurements.size)
        assertEquals(PerformanceSampleKind.Warm, measurements.single().sampleKind)
    ...
    fun deniedDiagnosticsConsentDoesNotStartUntilEligibilityIsRestored() {
        ...
        assertEquals(PerformanceSampleKind.Cold, measurements.single().sampleKind)
    }
}
